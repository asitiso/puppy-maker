...
export function completeTacticalTurn(session:BattleSession,actorId:string,units:TacticalUnit[]):BattleSession { const acted=[...session.acted,actorId],next:BattleSession={...session,units,acted};if(isBattleFinished(next))return next;const livingIds=units.filter(unit=>Number.isFinite(unit.hp)&&unit.hp>0).map(unit=>unit.id);return livingIds.every(id=>acted.includes(id))?refreshRound(next):next; }
...
export function resolveTacticalAction(session:BattleSession,input:TacticalActionInput):BattleSession {
 ...
 const actor=session.units.find(unit=>unit.id===input.actorId) as PoweredTacticalUnit|undefined;if(!actor||!Number.isFinite(actor.hp)||actor.hp<=0||!Number.isFinite(actor.ap)||!Number.isFinite(actor.mp))return session;
