...
import { journeyTierClaimKey, seasonJourneyKey, seasonJourneyTiers } from './season-journey';
...
describe('world progression reducer integration', () => {
  ...
  it('auto-pays regional renown and world contract rewards only once', () => {
    ...
    const maxJourneyScore = seasonJourneyTiers.at(-1)?.threshold ?? 0;
    const state = {
      ...initialState,
      ...
      seasonJourneyScores:{ [journeyKey]:maxJourneyScore },
      claimedSeasonJourneyTiers:seasonJourneyTiers.map(tier => journeyTierClaimKey(journeyKey,tier.tier)),
    };
