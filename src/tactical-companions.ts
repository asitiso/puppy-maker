...
const finiteNonNegative=(value:number)=>Number.isFinite(value)?Math.min(Number.MAX_SAFE_INTEGER,Math.max(0,value)):0
const safeRounded=(value:number,min=1)=>Number.isFinite(value)?Math.max(min,Math.min(Number.MAX_SAFE_INTEGER,Math.round(value))):min
...
export function deriveCompanionUnit(id: CompanionId, leader: LeaderCombatProgression): DerivedCompanionUnit {
  ...
  const maxHp = safeRounded(leaderMaxHp * 0.72 * (d.role === 'tank' ? 1.25 : d.role === 'support' ? 0.95 : 1))
  const power = safeRounded(leaderPower*0.68*roleScale)
  const magic = safeRounded(leaderMagic*(d.role==='support'?0.82:0.62))
  const skillPower = d.role==='support' ? magic : d.role==='striker' ? Math.max(power,magic) : safeRounded((power+magic)/2)
  const supportPower = d.role==='support' ? magic : safeRounded(magic*.65)
  return { id:`companion-${id}`, name:d.name, role:d.role, side:'ally', position:d.preferredPosition, maxHp, hp:maxHp,
    agility:safeRounded(leaderAgility*(d.role==='trickster'?1.05:0.82)), ap:3,maxAp:3,mp:0,maxMp:10,shield:0,
