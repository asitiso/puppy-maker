...
function TrainingFeedback({message}:{message:string}){
  return <p className="training-minigame__feedback" role="status" aria-live="polite" aria-atomic="true">{message||' '}</p>;
}
...
export default function TrainingActivityMinigame({schedule,year,month,week,score,combo,onTrain,onFinish}:Props){
  ...
  if(complete){
    return <section className="screen training-screen training-minigame is-complete" aria-label="이번 주 실습 완료">
      ...
      <div className="training-minigame__complete-card">
        ...
        <div className="training-minigame__complete-stats" aria-label="훈련 기록"><span>현재 점수 <strong>{score}</strong></span><span>최고 콤보 <strong>{combo}</strong></span></div>
        <button type="button" onClick={onFinish}>성장 결과 확인</button>
      </div>
    </section>;
  }
  ...
  if(activity==='hunt'){
    ...
    return <section className="screen training-screen training-minigame is-hunt" aria-label="사냥 훈련 미니게임">
      ...
      <div className="hunt-challenge"><small>{challengeMeta}</small><p>신호에 맞는 행동을 정확한 타이밍에!</p><strong>{expected.label}</strong><div className="hunt-meter"><span className="hunt-zone" style={{left:`${targetCenter*100}%`,width:`${challenge.timingWindow*200}%`}}/><i style={{left:`${needle*100}%`}}/></div><TrainingFeedback message={feedback}/></div>
  ...
  if(activity==='magic'){
    ...
    return <section className="screen training-screen training-minigame is-magic" aria-label="마법 수업 미니게임">
      ...
      <div className="magic-challenge"><small>RUNE MEMORY · {challengeMeta}</small><h2>{showMagicPattern?`${pattern.length}개의 룬 순서를 기억하세요`:'같은 순서로 룬을 입력하세요'}</h2><div className="magic-pattern">{showMagicPattern?pattern.join('  '):magicInput.map(()=> '●').join(' ')||'· · ·'}</div><TrainingFeedback message={feedback}/></div>
  ...
  return <section className="screen training-screen training-minigame is-herb" aria-label="약초 채집 미니게임">
    ...
    <div className="herb-challenge"><small>FIELD NOTE · {challengeMeta}</small><h2>{round<2?'설명에 맞는 약초를 고르세요':'핵심 특징만 보고 약초를 구별하세요'}</h2><p>{clue}</p><TrainingFeedback message={feedback}/></div>
