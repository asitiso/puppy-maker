...
export type DistrictDestination=
  ...
  |{kind:'questNpc';category:MobileContentCategory};
...
type DistrictSpec={
  ...
  questNpcPosition:{x:number;y:number};
};
...
export function parseDistrictDestination(destinationId:string):DistrictDestination|null{
  ...
  if(destinationId.startsWith('quest-npc:')){
    const category=destinationId.slice('quest-npc:'.length) as MobileContentCategory;
    return districtOrder.includes(category)?{kind:'questNpc',category}:null;
  }
  return null;
}
