...
function replayModel(state: GameState) {
  ...
  return {
    raising,
    world,
    model: buildNgPlusReplayViewModel({
      raising,
      world,
      currentRunEvents: ['이번 봄의 첫 훈련을 새로 마쳤어요.', '이번 회차의 탐험 기록이 시작됐어요.'],
    }),
  };
}
    ...
    runSummaries: [
      {
        ...
        campaign: 'pathfinder',
        route: 'normal',
        ending: 'old.pathfinder',
        career: 'pathfinder_career',
        majorWorldOutcomes: [],
        keyBondMemories: [],
        trueClues: ['pathfinder_world_route'],
      },
      {
        runNumber: 2,
        campaign: 'vanguard',
      ...
      },
      {
        ...
        route: 'normal',
        ending: 'old.arcanist',
        career: 'arcanist_career',
        majorWorldOutcomes: [],
        keyBondMemories: [],
        trueClues: ['arcanist_rift_cycle'],
      },
    ],
  };
}
...
describe('V3 NG+ final Wave cross-macro replay', () => {
  it('connects a real Winter ending through authoritative NEW_RUN into Macro A echoes, normal Spring convergence, UI and save/reload', () => {
    ...
    expect(spring.campaignRun).toMatchObject({
      runNumber: 2,
    ...
    expect(spring.legacy.ngPlusUnlocks).toEqual(expect.arrayContaining([
      'past_life_dialogue', 'relationship_reunion', 'world_echo',
    ]));
    ...
    const html = renderToStaticMarkup(
      <NgPlusReplayHub open model={replay.model} onOpen={() => undefined} onClose={() => undefined} />,
    );
    ...
    const reloadedReplay = replayModel(inspected.state);
    expect(reloadedReplay.model.normalCandidates.map(candidate => candidate.id))
      .toEqual(replay.model.normalCandidates.map(candidate => candidate.id));
    expect(reloadedReplay.model.journey.pastLife).toEqual(replay.model.journey.pastLife);
    expect(reloadedReplay.model.journey.worldEchoes).toEqual(replay.model.journey.worldEchoes);
  });

  it('surfaces canonically earned fifth_path_candidate only as an additive hook while normal campaigns remain available', () => {
    const base: GameState = {
      ...initialState,
      campaignRun: { ...initialState.campaignRun, runNumber: 4 },
      legacy: eligiblePriorLegacy(),
    };
    const completed = committedCaretakerEnding(base);
    expect(completed.legacy.completedRuns).toBe(4);
    expect(completed.legacy.completedCampaigns).toEqual(expect.arrayContaining([
      'caretaker', 'pathfinder', 'vanguard', 'arcanist',
    ]));

    const spring = reducer(completed, { type: 'NEW_RUN' });
    expect(spring.campaignRun.runNumber).toBe(5);
    expect(spring.legacy.completedRuns).toBe(4);
    expect(spring.legacy.runSummaries).toHaveLength(4);
    expect(spring.legacy.trueClues).toEqual(expect.arrayContaining([
