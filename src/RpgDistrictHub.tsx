...
import {clearWorldQuest,isQuestResolved,loadContinuousWorldQuest,loadWorldQuest,loadWorldQuestHistory,npcQuestProgress,questFromRecommendation,recordWorldQuestCompletion,saveContinuousWorldQuest,saveWorldQuest,worldQuestCampaignProgress,type WorldQuestContract,type WorldQuestHistory} from './world-quest-contract';
...
export default function RpgDistrictHub({category,state,onFeature,onBack}:Props){
  ...
  const targetCategory=trackedQuest?.category??recommendation.category;const targetLabel=trackedQuest?.title??recommendation.label;const targetReason=trackedQuest?.reason??recommendation.reason;const issuer=trackedQuest?questNpcForCategory(trackedQuest.category):questNpcForCategory(recommendation.category);const issuerCompleted=history.completedByIssuer[issuer.id]??0;const questProgress=npcQuestProgress(issuerCompleted);const campaign=worldQuestCampaignProgress(history.completed);const issuerJournal=history.recent.filter(entry=>entry.issuerId===issuer.id).slice(0,3);
  ...
  useEffect(()=>{if(!acceptedQuest||!resolved)return;clearWorldQuest(storage);const next=recordWorldQuestCompletion(storage,acceptedQuest);const npc=questNpcForCategory(acceptedQuest.category);const nextCampaign=worldQuestCampaignProgress(next.completed);setCompletionNotice(`${npc.name}: ${npc.completion} · 모험 챕터 ${nextCampaign.chapter} ${nextCampaign.completedInChapter}/${nextCampaign.chapterSize}`);setHistory(next);setAcceptedQuest(null);},[acceptedQuest,resolved]);
