...
import {clearWorldQuest,isQuestResolved,loadWorldQuest,loadWorldQuestHistory,npcQuestRank,questFromRecommendation,recordWorldQuestCompletion,saveWorldQuest,type WorldQuestContract,type WorldQuestHistory} from './world-quest-contract';
...
export default function RpgDistrictHub({category,state,onFeature,onBack}:Props){
  ...
  const targetCategory=trackedQuest?.category??recommendation.category;const targetLabel=trackedQuest?.title??recommendation.label;const targetReason=trackedQuest?.reason??recommendation.reason;const issuer=trackedQuest?questNpcForCategory(trackedQuest.category):questNpcForCategory(recommendation.category);const issuerCompleted=history.completedByIssuer[issuer.id]??0;
  ...
  return <section className="rpg-home-hub rpg-district-hub" aria-label={`${world.label} 월드 구역`}>
    ...
    <aside className="rpg-home-hub__tracker rpg-district-hub__tracker" aria-label="월드 의뢰 추적">
      <div className="rpg-district-hub__npc"><b>{issuer.name}</b><span>{issuer.role}</span><i>{npcQuestRank(issuerCompleted)} · {issuerCompleted}건</i></div>
