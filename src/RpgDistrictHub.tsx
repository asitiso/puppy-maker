...
export default function RpgDistrictHub({category,state,onFeature,onBack}:Props){
  ...
  useEffect(()=>{if(!continuous||acceptedQuest||resolved)return;const quest=questFromRecommendation(recommendation);saveWorldQuest(storage,quest);setAcceptedQuest(quest);},[continuous,acceptedQuest,resolved,recommendation.category,recommendation.feature]);
  ...
  const abandonQuest=()=>{clearWorldQuest(storage);setAcceptedQuest(null);};
  const toggleContinuous=()=>{const next=!continuous;saveContinuousWorldQuest(storage,next);setContinuous(next);setCompletionNotice(next?'연속 의뢰가 켜졌어요. 다음 추천부터 자동 수락합니다.':null);};
