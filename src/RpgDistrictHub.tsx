...
import {clearWorldQuest,isQuestResolved,loadWorldQuest,loadWorldQuestHistory,questFromRecommendation,recordWorldQuestCompletion,saveWorldQuest,type WorldQuestContract} from './world-quest-contract';
...
export default function RpgDistrictHub({category,state,onFeature,onBack}:Props){
  ...
  const [completedCount,setCompletedCount]=useState(()=>typeof window==='undefined'?0:loadWorldQuestHistory(window.localStorage).completed);
  ...
  const issuer=trackedQuest?questNpcForCategory(trackedQuest.category):questNpcForCategory(recommendation.category);
  const nextDistrict=nextDistrictToward(activeCategory,targetCategory);
  const routeHint=nextDistrict?`${districtLabel(nextDistrict)} 게이트로 이동`:`${districtLabel(activeCategory)} 안의 ${targetLabel} 시설로 이동`;
  ...
  const world={...baseWorld,objective:`${baseWorld.objective} ${issuer.name}의 ${priorityLabel}: ${targetLabel}. 다음 길: ${routeHint}.`};

  useEffect(()=>{
    if(!acceptedQuest||!resolved)return;
    const storage=typeof window==='undefined'?null:window.localStorage;
    clearWorldQuest(storage);
    const history=recordWorldQuestCompletion(storage,acceptedQuest);
    const npc=questNpcForCategory(acceptedQuest.category);
    setCompletionNotice(`${npc.name}: ${npc.completion}`);
    setCompletedCount(history.completed);
  ...
  const acceptQuest=()=>{
    ...
    setAcceptedQuest(quest);setCompletionNotice(null);
  };
  ...
  return <section className="rpg-home-hub rpg-district-hub" aria-label={`${world.label} 월드 구역`}>
    <MobileExplorationScene key={activeCategory} world={world} storyFrames={noStoryFrames} playerArtSrc={runaExplorationArt} onProgress={noop} onExit={onBack} onPortal={destinationId=>{
