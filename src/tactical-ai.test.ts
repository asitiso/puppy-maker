...
describe('tactical ai',()=>{
 ...
 it('auto does not waste Moonlight Prayer when no ally needs healing',()=>{
   const runa={...u('runa','ally'),agility:30,mp:10};
   const session=createBattleSession([runa,u('companion-owl','ally'),u('companion-bear','ally')],[u('enemy-a','enemy'),u('enemy-b','enemy'),u('enemy-c','enemy')],11);
   expect(chooseAutoCombinationUltimate(session,['owl'],{owl:5})).toBeNull();
 });
