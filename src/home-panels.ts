...
type LegacyHomeState={eventHistory?:string[];endingCollection?:EndingId[];claimedMilestones?:string[]};
...
export function getHomePanel(id:HomeMenuId,state:GameState):HomePanel|null{
 ...
 const history=storyHistory(state),endingCollection=endingHistory(state),milestones=milestoneCount(state);
 ...
 const endings=(Object.keys(endingNames) as EndingId[]).map(id=>({title:endingCollection.includes(id)?endingNames[id]:'???',detail:endingCollection.includes(id)?'엔딩 도감에 기록됨':'새로운 육성에서 발견할 수 있어요',state:endingCollection.includes(id)?'done' as const:'locked' as const}));
 ...
 const panels:Partial<Record<HomeMenuId,HomePanel>>={
  ...
  quest:{title:'육성 목표',eyebrow:'RAISING GOALS',summary:'다음 성장을 위한 장기 목표',items:[{title:'12개월 함께하기',detail:`${state.monthsCompleted}/12개월`,state:state.monthsCompleted>=12?'done':'active'},{title:'기억 도감 채우기',detail:`${state.memories.length}/${Object.keys(MEMORY_CATALOG).length}`,state:state.memories.length===Object.keys(MEMORY_CATALOG).length?'done':'active'},{title:'이야기 만나기',detail:`${history.length}/${STORY_EVENTS.length}`,state:history.length===STORY_EVENTS.length?'done':'active'}]},
  outing:{title:'엔딩의 별자리',eyebrow:'DESTINY',summary:`현재 가장 가까운 미래 · ${endingNames[leading]}`,items:endings},
  attendance:{title:'함께한 시간',eyebrow:'JOURNEY',summary:`루나와 ${state.monthsCompleted}개월째 성장 중`,items:[{title:`${state.year}년 ${state.month}월`,detail:`${state.week}주차 · ${condition(state)}`,state:'active'},{title:'성장 이정표',detail:`${milestones}/4 달성`,state:milestones===4?'done':'active'}]},
