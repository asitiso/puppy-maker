...
type LegacyHomeState={eventHistory?:string[];endingCollection?:EndingId[]};
const storyHistory=(state:GameState)=>{const legacy=(state as GameState&LegacyHomeState).eventHistory;if(Array.isArray(legacy))return legacy;return currentStoryChapters(state).map(String);};
const endingHistory=(state:GameState)=>{const legacy=(state as GameState&LegacyHomeState).endingCollection;return Array.isArray(legacy)?legacy:[];};
export function getHomePanel(id:HomeMenuId,state:GameState):HomePanel|null{
 ...
 const history=storyHistory(state),endingCollection=endingHistory(state);
